...
function HomeCard({ img, title, description, auth, genre }) {
  ...
  const borrowBook = async (borrowedBookTitle) => {
    ...
    if (borrowedBook && borrowedBook.isBorrowed) {
      ...
      toast.success(`Livre rendu : ${borrowedBookTitle}`);
      loadBooks();
      ...
      toast.success(`Livre emprunté : ${borrowedBookTitle}`);
      loadBooks();
    } else if (borrowedBook) {
      toast.success(`Stock épuisé pour le livre : ${borrowedBookTitle}`);
    } else {
      toast.success(`Livre non trouvé avec l'ID : ${borrowedBookTitle}`);
    }
  };
